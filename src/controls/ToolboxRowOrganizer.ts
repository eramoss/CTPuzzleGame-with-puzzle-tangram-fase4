...
export default class ToolboxRowOrganizer {
    flow: FlexFlow;
    texturesReservations: CommandName[];
    commandsAddedTextureKey: Array<string>
    scaleNormal: number;
    scaleOnPointerOver: number
    scaleOnDragStart: number;

    constructor(grid: AlignGrid, cellx: number, celly: number, rowspan: number, colspan: number, commandNames: Array<CommandName>, scaleNormal: number = 1) {
