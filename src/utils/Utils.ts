...
export function createDropZone(
  ...
  const rect: Phaser.Geom.Rectangle = grid.getArea(
    cellx,
    celly,
    colspan,
    rowspan
  );
  const dropZone = new SpriteDropZone(
    grid.scene,
    rect.x,
    rect.y,
    rect.width,
...
export function getDefaultPlatformApiUrl(gameParams: GameParams) {
  ...
  apiUrl = apiUrl || defaultOnlineApi;
  return apiUrl;
}
 ...
 */
export function joinChilds<PARENT, CHILD>(
  parents: Array<PARENT>,
  fnGetChilds: (p: PARENT) => Array<CHILD>
): Array<CHILD> {
...
export function createJoinArraysFn<PARENT>(
  functions: Array<() => PARENT[]>
): () => Array<PARENT> {
  return () =>
    joinChilds(functions, (fnThatReturnArray) => fnThatReturnArray());
}
