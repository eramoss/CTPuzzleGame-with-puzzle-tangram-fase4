...
export default class MazeConfigs {
  ...
  async loadTestFromServer(ctPuzzlePlatformUrl: string): Promise<MazeConfigs> {
    let test: Response = await fetch(ctPuzzlePlatformUrl)
    let json = await test.json()

    this.phases = new Array<MazePhase>();
    json.items.forEach((testItem: any) => {
      let phase = new MazePhase(this.scene, this.codeEditor);
      let itemDefinition = testItem.item.itemDefinition;
      console.log("PARSING_ITEM\n", itemDefinition)
      let mecanicaRope = JSON.parse(itemDefinition);
      phase.mecanicaRope = mecanicaRope as MecanicaRope;
  ...
  createHardCodedPhases(): MazeConfigs {
    this.phases = new Array<MazePhase>();
